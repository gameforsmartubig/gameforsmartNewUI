...
export async function POST(
  ...
  try {
    ...
    // Unblock the quiz (set is_hidden to false and clear hidden_at and hidden_reason)
    ...
    if (unblockError) {
      console.error("Unblock quiz error:", unblockError);
      return NextResponse.json(
        { error: `Failed to unblock quiz: ${unblockError.message}` },
        { status: 500 }
      );
    }
    ...
    try {
      await supabase.from("admin_actions").insert({
        admin_id: user.id,
        action_type: "quiz_unblock",
        target_user_id: quiz.creator_id,
        target_content_type: "quiz",
        target_content_id: quizId,
        description: `Quiz unblocked by admin: "${quiz.title}" owned by ${ownerInfo}`,
        reason: "Quiz unblocked",
        metadata: {
    ...
    return NextResponse.json({
      success: true,
      message: `Quiz "${quiz.title}" has been successfully unblocked`,
      data: {
        unblockedQuizId: quizId,
        unblockedQuizTitle: quiz.title,
        quizOwner: ownerInfo,
