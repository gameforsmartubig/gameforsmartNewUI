...
export async function PATCH(
  ...
  try {
    ...
    if (!action || !['accept', 'decline', 'block', 'unblock'].includes(action)) {
      return NextResponse.json({ error: 'Valid action (accept/decline/block/unblock) is required' }, { status: 400 });
    }
    ...
    if (action === 'accept') {
      ...
      if (friendship.addressee_id !== userIdStr) {
        return NextResponse.json({ error: 'Only the addressee can accept requests' }, { status: 403 });
      }
      ...
      return NextResponse.json({
        message: isMutual ? 'Follow request accepted - You are now friends!' : 'Follow request accepted',
        data,
        mutual: isMutual
      });
      ...
      if (friendship.addressee_id !== userId) {
        return NextResponse.json({ error: 'Only the addressee can decline requests' }, { status: 403 });
      }
      ...
      const { data, error: updateError } = await supabase
        ...
        .update({ 
          ...
          addressee_id: friendship.requester_id === userId ? friendship.addressee_id : friendship.requester_id,
          updated_at: new Date().toISOString()
        })
        .eq('id', followId)
        .select()
