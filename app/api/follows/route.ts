  ...
  try {
    ...
    switch (normalizedType) {
      ...
      case "requests_sent":
        ...
        // Filter by search if provided
        const filteredSentRequests = search
          ? processedSentRequests.filter(
              (user: any) =>
                user.username?.toLowerCase().includes(search.toLowerCase()) ||
                user.fullname?.toLowerCase().includes(search.toLowerCase())
            )
      ...
      case "friends":
        ...
        const followingIdMap = new Map(userFollowingData.map((f) => [f.addressee_id, f.id]));
      ...
      case "blocked":
        ...
        // Filter by search if provided
        const filteredBlocked = search
          ? processedBlocked.filter(
              (user: any) =>
                user.username?.toLowerCase().includes(search.toLowerCase()) ||
                user.fullname?.toLowerCase().includes(search.toLowerCase())
            )
      ...
      default:
        ...
        console.error("Invalid type parameter received:", {
          type: receivedType,
          normalized: receivedNormalized,
          userId
        });
        return NextResponse.json(
          {
  ...
  try {
    ...
    if (existingFollow) {
      if (existingFollow.status === "accepted") {
        ...
        return NextResponse.json({ error: "Follow request already sent" }, { status: 409 });
      } else if (existingFollow.status === "blocked") {
        return NextResponse.json({ error: "Cannot follow blocked user" }, { status: 409 });
      }
    }
    ...
    try {
      ...
      if (followerFullProfile) {
        const followerName =
          followerFullProfile.fullname || followerFullProfile.username || "Someone";
