...
export async function GET(request: NextRequest) {
  try {
    ...
    let searchQuery = supabase
      .from('profiles')
      .select('id, username, fullname, nickname, avatar_url, last_active, country_id, state_id, city_id, countries (name), states (name), cities (name)')
    ...
    const processedResults = users.map(user => {
      ...
      const isFollowing = followRelations?.some(rel => 
      ...
      const isFollowedBy = followRelations?.some(rel => 
      ...
      const areFriends = followRelations?.some(rel => 
        rel.requester_id === userIdStr && 
        rel.addressee_id === userIdString && 
        rel.status === 'accepted'
      ) && followRelations?.some(rel => 
        rel.requester_id === userIdString && 
        rel.addressee_id === userIdStr && 
        rel.status === 'accepted'
      );
      ...
      const isBlocked = followRelations?.some(rel => 
        rel.requester_id === userIdStr && 
        rel.addressee_id === userIdString && 
        rel.status === 'blocked'
      ) || false;
      ...
      const isBlockedBy = followRelations?.some(rel => 
        rel.requester_id === userIdString && 
        rel.addressee_id === userIdStr && 
        rel.status === 'blocked'
      ) || false;
      ...
      return {
        ...
        is_online: user.last_active && 
          new Date(user.last_active) > new Date(Date.now() - 15 * 60 * 1000),
