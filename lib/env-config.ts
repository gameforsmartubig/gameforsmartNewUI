...
export function validateClientEnv(): { isValid: boolean; errors: string[] } {
  ...
  if (
    ...
    !clientEnv.supabase.url.match(/^https:\/\/[a-z0-9]+\.supabase\.co$/)
  ) {
...
export function validateServerEnv(): { isValid: boolean; errors: string[] } {
  ...
  if (process.env.NODE_ENV === "production") {
    if (!serverEnv.supabase.serviceRoleKey) {
      errors.push(
        "NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY is required in production"
      );
    }
  ...
  return {
    client: {
      supabase: {
        ...
        anonKey: clientEnv.supabase.anonKey
          ? "***" + clientEnv.supabase.anonKey.slice(-10)
          : "NOT_SET"
      },
      app: clientEnv.app
    },
    server: {
      ...
      security: {
        ...
        corsOrigins:
          serverEnv.security.corsOrigins.length > 0
            ? `${serverEnv.security.corsOrigins.length} origins`
            : "NOT_SET"
      }
    },
