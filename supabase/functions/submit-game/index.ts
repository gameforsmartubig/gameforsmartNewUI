...
async function syncSingleParticipantToMain(mainClient: any, sessionId: string, participant: any) {
  ...
  // 3. Prepare response data (Grouped by Participant)
  const participantResponses = {
    user_id: participant.user_id,
    participant: participant.id,
    answers: (participant.responses || []).map((r: any) => ({
      id: r.id,
  ...
  existingResponses = existingResponses.filter((r: any) => r.participant !== participant.id);
 ...
 */
async function syncAllParticipantsToMain(
  mainClient: any,
  sessionId: string,
  allParticipants: any[],
  limit: number
  ...
  // 2. Prepare ALL responses in grouped format (New Structure)
  const allResponses = allParticipants.map((p: any) => {
    return {
      user_id: p.user_id,
      participant: p.id,
      answers: (p.responses || []).map((r: any) => ({
        id: r.id,
        answer_id: r.answer_id,
        question_id: r.question_id
      }))
  ...
  console.log(
    `✅ Synced ${participantsData.length} participants (Clean, Normalized) and ${allResponses.length} responses to Main DB`
  );
}
